...
public class Scenes
{
	...
	private static String[] columnsKeys = {"Score", "CPM", "Difficulty", "Language", "Game mode", "DateTime", "Name"};
	...
	public static Scene scoreboard(boolean _saved)
	{
		...
		KeyButton sortButton = new KeyButton(scene, root, KeyCode.TAB, "TAB", 25, 447, 50, 20, 14, 8,
				new ButtonAction()
				{
					@Override
		...
		KeyButton deleteButton = new KeyButton(scene, root, KeyCode.DELETE, "DEL", 25, 472, 50, 20, 14, 8,
				new ButtonAction()
				{
					...
					public void callback(Pane root, boolean active)
					{
						...
						} else deleteText.setText("SAVE FIRST");
					}
				});
	...
	private static void setToDeleteEntry(int index, Pane resultsContainer, Text pageNumber)
	{
		...
		if (index != -1) entries.get(index).setToDelete(true);
		showScoreboardPage(currentPage, resultsContainer, entries, pageNumber);
	}
	...
	public static Pane selectMenu(String type)
	{
		...
		{
			case "GAMEMODE":
				root.getChildren().addAll(game_modes);
				break;
