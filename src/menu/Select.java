...
public class Select
{
	...
	public static void selectGamemode()
	{
		...
		scene.setOnKeyPressed(e -> {
			switch (e.getCode())
			{
				...
				case ENTER:
					Window.gameMode = row;
					Log.success("Gamemode: " + Scenes.game_modes[row].getValue());
	...
	private static void customDifficulty(boolean infinite)
	{
		...
		scene.setOnKeyPressed(e -> {
			...
			switch (e.getCode())
			{
				...
				case DOWN:
					if (row++ < amount - 1) Scenes.pointer.setTranslateY(arrowY + 30);
					else
					{
						row = 0;
						Scenes.pointer.setTranslateY(arrowY - (amount - 1) * 30);
				...
				case RIGHT:
					if ((x[row] + 1) < 10) Scenes.scales[row][++x[row]].setFilled(true);
					break;

				case LEFT:
					if (x[row] > 0) Scenes.scales[row][x[row]--].setFilled(false);
					break;
	...
	{
		int index = 0;
		for (Option o : tab) o.setHighlighted(index++ == row);
	}
